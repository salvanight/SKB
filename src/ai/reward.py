...
def calculate_reward(previous_context: Dict[str, Any],
    ...
    # --- Bonus for Proactive Healing ---
    # Ensure prev_hp and curr_hp are already defined as they are earlier in the function.
    # Action enum should also be available.
    try:
        action_taken_for_healing_check = Action(action_id)
        if action_taken_for_healing_check == Action.USE_HEALTH_POTION:
            if prev_hp < 0.6 and curr_hp > prev_hp: # HP was low (e.g., <60%) and increased
                healing_bonus = 15.0 # Specific bonus for this successful proactive heal
                reward += healing_bonus
                # print(f"DEBUG: Healing bonus applied: {healing_bonus}") # Optional debug
    except (NameError, ValueError): # Action enum might not be loaded if there was an import issue
        pass
...
if __name__ == '__main__':
    ...
    # Expected: (1.0-0.2)*100 (hp_reward) + 15.0 (healing_bonus) + 0.1 (action_liveness) = 80 + 15 + 0.1 = 95.1
    print(f"Test 4 (Used potion, healed from low HP): Reward = {r4}")
    ...
    # Test 7: Used health potion but HP was not low (e.g. > 0.6)
    context_hp_high_no_target = {
        'ng_statusBar': {'hpPercentage': 0.7},
        'ng_cave': {'targetCreature': None, 'isAttackingSomeCreature': False}
    }
    context_hp_very_high_no_target = {
        'ng_statusBar': {'hpPercentage': 0.9},
        'ng_cave': {'targetCreature': None, 'isAttackingSomeCreature': False}
    }
    r7 = calculate_reward(context_hp_high_no_target, context_hp_very_high_no_target, Action.USE_HEALTH_POTION)
    # Expected: (0.9-0.7)*100 (hp_reward) + 0.1 (action_liveness) = 20 + 0.1 = 20.1 (No healing bonus)
    print(f"Test 7 (Used potion, but HP not low): Reward = {r7}")

    # Test 8: Used health potion, HP was low, but HP did not increase (e.g., potion on cooldown or out of potions)
    context_hp_low_remains_low = {
        'ng_statusBar': {'hpPercentage': 0.3}, # Same as prev_hp for this sub-test
        'ng_cave': {'targetCreature': None, 'isAttackingSomeCreature': False}
    }
    prev_context_for_failed_heal = { # HP is low
        'ng_statusBar': {'hpPercentage': 0.3},
        'ng_cave': {'targetCreature': None, 'isAttackingSomeCreature': False}
    }
    r8 = calculate_reward(prev_context_for_failed_heal, context_hp_low_remains_low, Action.USE_HEALTH_POTION)
    # Expected: (0.3-0.3)*100 (hp_reward) + 0.1 (action_liveness) = 0 + 0.1 = 0.1 (No healing bonus)
    print(f"Test 8 (Used potion, HP low, but no HP increase): Reward = {r8}")
